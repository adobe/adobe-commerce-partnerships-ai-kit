...
concrete reference (see §3.3) rather than inventing new ones.
...
implemented, and prescribes adding `renewedQuantity?: number` to `models/Subscription.ts`. **This UI
LLD depends on that backend LLD's `models/Subscription.ts` change being applied** — every
early-renewal UI behavior below (window-rollover check, segmentation, badge, quantity clamp) reads
...
(AD), from the Customer Details page. The feature adds one new entry point and one new dialog to
the existing Products tab:

1. **Early Renew entry point** — an `ActionButton` in `CustomerProductsOverviewPanel`'s Anniversary
   Date row, shown only when the customer is inside its early-renewal window.
2. **Early Renewal Order Dialog** — a new dialog where the partner reviews eligible subscriptions
   ...
   line, and submits an actual `RENEWAL` order.
...
directly** rather than forking. Only the genuinely new pieces (AD-window eligibility, the
renewedQuantity-based rollover check, and Existing/Additional segmentation) are new code, isolated
in a new `utils/earlyRenewalUtils.ts` and a new `EarlyRenewalOrderDialog` component that otherwise
mirrors `LateRenewalOrderDialog`'s structure closely.

User journey: partner opens a customer's detail page → (if in window) sees an "Early renew" button
next to "Edit renewal order" → clicks it → dialog loads eligible subscriptions, splits them into
Existing/Additional, and fetches a price preview for the active segment → partner adjusts
quantities/toggles/discount codes → partner clicks "Renew now" → order is placed → dialog closes,
a success toast shows the order ID, and the subscriptions query is invalidated so the Products list
reflects the new `renewedQuantity` without a manual page refresh.
...
|------|--------|-------|--------|
| `utils/earlyRenewalUtils.ts` | CREATE | Util | AD-window eligibility check, renewedQuantity-based rollover check, Existing/Additional segmentation, Existing-segment and Additional-segment quantity/toggle initializers, deadline formatting — none of this exists today |
...
No route changes (feature lives entirely within the existing `/customerdetails` page) and no new
data fetch unit beyond what's reused from `utils/renewalOrderUtils.ts` — see §4 for why reuse was
chosen over forking a parallel `fetchEarlyRenewalOrderPreview`/`placeEarlyRenewalOrder`.
 ...
 *  - at least one subscription already has a populated renewedQuantity (>= 0) — meaning an
 *    early renewal was already placed this cycle, which is why AD has rolled forward. Checking
 *    renewedQuantity directly (rather than re-deriving a shifted day window) is simpler and does
 *    not need to track how many times AD has rolled.
 */
...
- `computeSubscriptionsEligibleForRenewalUpdate` (status 1000/1009 filter) is **not** redefined
  here — it's imported from `utils/customerDetailsUtils.ts` at the call site in the dialog.
...
4. Mount the dialog near the existing `EditRenewalDialog`/`LateRenewalOrderDialog` mounts:
   ...
   cache invalidation happens inside the dialog itself, see §3.2 and §4).
...
| Error codes 2164 / 3115 / 3121 / 3131 / 3132 / 3133 surfaced to the partner (no per-code translation) | `placeRenewalOrder`/`fetchRenewalOrderPreview` (reused, already throw the backend's verbatim message) → `ErrorToast` / `onError` |
...
## §7 — Addendum: Surface 3 — Add New Product to Early Renewal (Catalog)

This addendum covers the experience card's Surface 3: adding a **brand-new** product (one the
customer doesn't already own) to an early renewal, starting from the Catalog's existing
"attach a customer to this cart" step rather than from Customer Details. Backend: no change — see
backend LLD §2 Operation 4 and §5. This addendum reuses `fetchRenewalOrderPreview` / `placeRenewalOrder`
(`utils/renewalOrderUtils.ts`) verbatim, the same way §3's `EarlyRenewalOrderDialog` does, and reuses
`isInEarlyRenewalWindow` / the new `hasEarlyRenewalOrderPlaced` from `utils/earlyRenewalUtils.ts`.

**Note on how this was derived:** a structurally similar flow already exists in a separate,
full-featured sibling app (a "Now (Early Renewal)" license-availability option, routed to a
dedicated early-renewal checkout page). This addendum was **derived independently** against
*this* reference app's actual current architecture — its own `FindOrCreateCustomer` /
...
#### Flow 1 — "Make licenses available" eligibility (existing-customer step of Find/Create Customer)
...
Trigger: partner has a product in the cart, opens "Add customer details", stays on the
  ...
  → isInEarlyRenewalWindow(customerDetails.cotermDate, eligibleSubscriptions)
    [utils/earlyRenewalUtils.ts, REUSED — same 1–30-day / rolled-over check as Surface 1]
  ...
  → hasEarlyRenewalOrderPlaced(eligibleSubscriptions)   [utils/earlyRenewalUtils.ts, NEW — extracted
    from isInEarlyRenewalWindow's existing "already rolled over" check; same renewedQuantity-populated
    predicate, now independently reusable]
  ← "Now (Early Renewal)" radio is disabled when NOT isInEarlyRenewalWindow OR customerOwnsAnyCartOffer
  ...
  ← no network call beyond the two reads above — this is otherwise a pure client-side derivation,
    same posture as Flow 1 in §2
```

#### Flow 2 — Continue with "Now (Early Renewal)" selected

```
Trigger: partner selects the "Now (Early Renewal)" radio and clicks "Continue" in
...
#### Flow 3 — Early renewal checkout page loads

```
Trigger: pages/checkout/renewal.tsx mounts (customerInfoInCart already populated from Flow 2)
  ...
  → hasEarlyRenewalOrderPlaced(computeSubscriptionsEligibleForRenewalUpdate(subscriptions))
    [utils/earlyRenewalUtils.ts, REUSED from Flow 1 — determines whether THIS order will be the one
  ...
  → fetchRenewalOrderPreview(customerId, lineItems, currencyCode)   [utils/renewalOrderUtils.ts,
    REUSED from §3 — POST /api/orders?type=PreviewRenewal, orderType: "PREVIEW_RENEWAL"]
      → backend: see backend LLD §2 Operation 4 (unchanged)
  ← response: { lineItems[].pricing, lineItems[].proratedDays, pricingSummary[] }
...
#### Flow 4 — Submit ("Place order" on the early-renewal checkout)

```
Trigger: partner clicks "Place order" on pages/checkout/renewal.tsx

  → placeRenewalOrder(customerId, externalReferenceId, currencyCode, lineItems)
    [utils/renewalOrderUtils.ts, REUSED from §3 — POST /api/orders?type=RenewalOrder,
    orderType: "RENEWAL"]
      → backend: see backend LLD §2 Operation 4 (unchanged)
  ← response: { orderId, status, lineItems[], links }
  ...
  ← on failure: same ErrorToast pattern as /checkout today, backend message surfaced verbatim (no
    per-code translation — same posture as Surfaces 1–2, see backend LLD §5)
```
...
| `utils/earlyRenewalUtils.ts` | MODIFY | Util | Extract `hasEarlyRenewalOrderPlaced` out of `isInEarlyRenewalWindow` so the Catalog flow can call it independently; add `customerOwnsAnyCartOffer` (new-products-only guard) |
| `components/catalogCart/customerdetails/ExistingCustomer.tsx` | MODIFY | Component | Add the "Now (Early Renewal)" radio, fetch the selected customer's subscriptions (not fetched here today), compute the two eligibility guards, disable/auto-switch radios accordingly, show helper text |
...
No backend files change (see backend LLD §2 Operation 4 and §3). No change to `pages/catalog.tsx` — the "Add customer
details" entry point into `FindOrCreateCustomer` is unchanged; everything new lives inside the
existing-customer step and the new checkout route.
...
### `utils/earlyRenewalUtils.ts` — MODIFY

```ts
/**
 * True if at least one subscription already has a populated renewedQuantity (>= 0) — meaning an
 * early renewal was already placed this cycle. Extracted out of isInEarlyRenewalWindow so the
 * Catalog entry point can ask this question on its own, independent of the day-window check.
 */
 ...
 * owns, that product must be renewed/added-to from the Customer Details dialog (Surface 2) instead.
 */
...
- `isInEarlyRenewalWindow`'s existing behavior and signature are unchanged — only its internal
  "already rolled over" check is now delegated to the newly-exported `hasEarlyRenewalOrderPlaced`,
  so Surface 1's `CustomerProductsOverviewPanel` call site needs no change.
- `customerOwnsAnyCartOffer` imports `getSKUFromOfferId` from `utils/commonUtils.ts` (REUSED) — the
  same SKU-truncation helper already used for cart/subscription matching elsewhere in this repo.
...
`anniversaryDate` display string used elsewhere in this file — matches how Surface 1 calls it.
...
| Place-order call | `useCreateOrder()` mutation → `type=NEW` | `placeRenewalOrder` (`utils/renewalOrderUtils.ts`, REUSED), called directly inside a local async handler with local `isPlacingOrder` state (this function is a plain async call, not a `useMutation` hook, matching how `EarlyRenewalOrderDialog` already calls it in §3) |
...
surface `error.message` verbatim (no per-code translation, consistent with Surfaces 1–2).
...
| `hasEarlyRenewalOrderPlaced` extracted out of `isInEarlyRenewalWindow` into its own exported function | Surface 3 needs to ask "has an early renewal already started for this contract" independently of the day-window check (it drives disabling `Now`, not `Now (Early Renewal)`) — inlining it was fine when only one caller (`isInEarlyRenewalWindow`) existed, but a second independent caller means it should be named and reusable on its own | None — `isInEarlyRenewalWindow`'s public behavior is unchanged, this is a pure internal refactor | `utils/earlyRenewalUtils.ts` |
...
### §7.4 — Acceptance Criteria Coverage

| Goal (from experience card, Surface 3) | Covered by |
|-----------------------------------------|-----------|
...
| Partners cannot create an ambiguous order mixing new products with owned products | `utils/earlyRenewalUtils.ts` (`customerOwnsAnyCartOffer`) disables "Now (Early Renewal)" whenever the cart contains something the customer already owns |
...
| Accurate, prorated pricing shown before confirming | `fetchRenewalOrderPreview` (REUSED), `prorationDays` bullet in `CheckoutReviewDetails` |
...
| Clear success/failure feedback | `placeRenewalOrder` (REUSED) → existing `/orderConfirmation` redirect (success) / `ErrorToast` (failure), same posture as `/checkout` today |
